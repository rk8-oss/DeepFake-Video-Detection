...
def get_video_name_from_filename(filename):
    """Extract video name from filename with proper error handling."""
    video_name_parts = filename.split("_")
    
    # Handle the case where there aren't enough parts
    if len(video_name_parts) < 2:
        # Return the filename without extension as fallback
        return os.path.splitext(filename)[0]
    
    # Return original logic - first two parts joined with underscore
    return video_name_parts[0] + "_" + video_name_parts[1]


def plot_clusters(features, img_names, labels, method_name, output_path):
    """Plot clusters with different colors for different file prefixes."""
    # Reduce dimensions to 2D using PCA for visualization
    pca = PCA(n_components=2)
    features_2d = pca.fit_transform(features)
    
    # Get prefixes from image names
    prefixes = []
    for name in img_names:
        if name.startswith('YT_'):
            prefixes.append('YT')
        elif name.startswith('DF_'):
            prefixes.append('DF')
        else:
            prefixes.append('Other')
    
    # Create color maps
    prefix_colors = {'YT': 'green', 'DF': 'Red', 'Other': 'blue'}
    cluster_colors = {0: 'orange', 1: 'purple'}
    ...
    for i in range(len(features_2d)):
        ...
        # Plot with cluster as shape and prefix as color
        plt.scatter(x, y, 
                    c=prefix_colors[prefix], 
                    marker='o' if cluster == 0 else 's',
                    alpha=0.7,
                    edgecolors='w')
    
    # Create legends for prefixes
    prefix_patches = [mpatches.Patch(color=color, label=prefix) 
                      for prefix, color in prefix_colors.items()]
    
    # Create legends for clusters using Line2D
    from matplotlib.lines import Line2D
    cluster_patches = [
        Line2D([0], [0], marker='o', color='w', markerfacecolor='black', markersize=10, label='Cluster 0'),
        Line2D([0], [0], marker='s', color='w', markerfacecolor='black', markersize=10, label='Cluster 1')
    ]
    
    plt.legend(handles=prefix_patches + cluster_patches, loc='upper right')
    plt.title(f'Clustering Visualization ({method_name})\nPoints colored by file prefix, shaped by cluster')
    plt.xlabel('PCA Component 1')
    plt.ylabel('PCA Component 2')
    
    # Save plot
    plot_filename = os.path.join(output_path, f'clustering_{method_name.lower()}.png')
...
def main():
    ...
    kmeans_labels = kmeans.fit_predict(vaf_array)
    
    # Plot KMeans results
    plot_clusters(vaf_array, img_name_list, kmeans_labels, 'KMeans', output_path)
    
    # 2. Gaussian Mixture Model
    print("\nRunning Gaussian Mixture Model clustering...")
    gmm = GaussianMixture(n_components=2, random_state=42)
    gmm_labels = gmm.fit_predict(vaf_array)
    
    # Plot GMM results
    plot_clusters(vaf_array, img_name_list, gmm_labels, 'GMM', output_path)
    
    # Use KMeans results for pseudo-labeling (you can change this to GMM if it works better)
    cluster_label = kmeans_labels

    # Assignment Pseudo-labels
    data = list(zip(img_name_list, cluster_label))
    print("\nSample cluster assignments:")
    print(data[:10])  # Print first 10 samples

    video_label_counts = {}

    for image, label in data:
        # Use the helper function to safely extract video name
        video_name = get_video_name_from_filename(image)
    ...
    for filename in os.listdir(unlabeled_data_path):
        ...
        # Use the helper function to safely extract video name
        video_name = get_video_name_from_filename(filename)
    ...
    print(f"\nThe data has successfully been written to the file: {os.path.join(output_path, output_file)}")
